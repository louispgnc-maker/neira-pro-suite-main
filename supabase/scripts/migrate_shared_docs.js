#!/usr/bin/env node
/*
  Script de migration : copie les fichiers référencés par `cabinet_documents` (et liés à `documents`)
  ...
  Usage:
    ...
    node supabase/scripts/migrate_shared_docs.js

  Options:
    --dry-run   : n'effectue pas d'upload ni de mise à jour en base, affiche ce qui serait fait

  Remarques:
  - Nécessite la clé service_role (gardez-la secrète).
  - Le script tente d'utiliser `shared-documents` puis `shared_documents`.
  - Il ignore les entrées qui ont déjà une URL publique (commence par http).
*/
...
async function fetchDocsToMigrate() {
  // Select cabinet_documents rows that reference a document_id and don't already have a public file_url
...
async function main() {
  ...
  for (const row of rows) {
    try {
      ...
      // skip if file_url already looks public
      ...
      // create a short signed url to download the object from documents
      ...
      for (const b of BUCKET_CANDIDATES) {
        ...
        // if bucket not found or other error, try next
      ...
      // get public url and update cabinet_documents
      let publicUrl = null;
      if (dryRun) {
