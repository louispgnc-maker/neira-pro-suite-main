...
-- Add a uuid[] column to hold attached cabinet_documents ids to avoid computed/view issues
alter table if exists public.cabinet_dossiers
  add column if not exists attached_document_ids uuid[] default '{}'::uuid[];

-- Ensure column is not null for simpler client logic
-- Backfill: ensure existing rows have an array before enforcing NOT NULL
update public.cabinet_dossiers set attached_document_ids = '{}'::uuid[] where attached_document_ids is null;
...
create index if not exists idx_cabinet_dossiers_attached_ids_gin on public.cabinet_dossiers using gin (attached_document_ids);
