...
interface CabinetNotificationsListProps {
  types: ('cabinet_message' | 'cabinet_document' | 'cabinet_dossier' | 'cabinet_contrat' | 'cabinet_client')[];
  title?: string;
  emptyMessage?: string;
}

export function CabinetNotificationsList({ types, title = "Notifications", emptyMessage = "Aucune notification" }: CabinetNotificationsListProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`cabinet_notifications_list_${types.join('_')}`)
      .on(
        'postgres_changes',
        {
          event: '*',
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>
              {unreadCount > 0 ? `${unreadCount} nouvelle(s)` : 'Toutes lues'}
            </CardDescription>
          </div>
