...
/*
  ...
  Usage:
    ...
    SUPABASE_URL=... SUPABASE_SERVICE_ROLE_KEY=... node tools/migrate-shared-bucket.cjs --run
...
const migrateScript = path.resolve(__dirname, '..', 'supabase', 'scripts', 'migrate_shared_docs.cjs');
const backfillScript = path.resolve(__dirname, '..', 'supabase', 'scripts', 'backfill_shared_docs.cjs');
...
let res = spawnSync('node', [migrateScript, '--dry-run'], { stdio: 'inherit', env: process.env });
if (res.status !== 0) {
...
res = spawnSync('node', [backfillScript, '--dry-run'], { stdio: 'inherit', env: process.env });
if (res.status !== 0) {
  print('\nbackfill script failed with status ' + res.status + '. Aborting.');
  process.exit(res.status || 1);
}

print('\nMigration/backfill completed (wrapper executed dry-run steps).');
process.exit(0);
